...
@include('sweetalert::alert')
    <div class="container">
        <div class="row m-0">
            <div class="col p-0 ">
                <div class="box-left">
                    <p class="textmuted h8 mb-2">Price Order G-Academy | <a href="{{ route('carts.index') }}">Kembali</a></p>
                    ...
                    <div class="h8">
                        <div class="row m-0 border mb-3">
                            ...
                            <div class="col-6 text-end pr-2">
                                ...
                                @foreach ($data['cart']['data'] as $k=>$v)
                                    ...
                                    <span class="d-block py-2 border-bottom">Rp <span id="{{ $v['data_class']['class']['id'] }}">{{ number_format($v['price']) }}</span></span>
                                @endforeach
                            </div>
                        </div>
                        <div class="d-flex h7 mb-4">
                            ...
                            <p class="ms-auto" id="total-price">Rp {{ empty($total) ? number_format($data['cart']['price_total']) : number_format($total)}}</p>
                        </div>
                    </div>
                    <div class="">
                        ...
                        @if($status == 0)
                            <div class="alert alert-danger" role="alert">Masa aktif voucher sudah habis</div>
                        @endif
                        @if($status == 2)
                            <div class="alert alert-danger" role="alert">Voucher tidak tersedia/sudah terpakai</div>
                        @endif

                        <div class="form">
                            <div class="row">
                                <div class="col-12">
                                    <form method="post" action="{{ route('carts.redeem') }}">
                                    @csrf
                                        <div class="card border-0">
                                            <input class="form-control ps-5" type="text" id="code-voucher" name="code_voucher" placeholder="Kode Voucher" required> <span class="far fa-credit-card"></span>
                                        </div>
                            ...
                            @csrf
                                ...
                                <div class="d-grid gap-2">
                                    <button class="btn btn-primary d-block h8 mt-4" id="order">BAYAR SEKARANG <span class="ms-3 fas fa-arrow-right"></span></button>
                                </div>
                            </form>
                        </div>
                    </div>
